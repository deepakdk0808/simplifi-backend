  ...
  async function sendOTPToMobile(mobile, otp, isdCode) {
    try {
  ...
  router.post(
    ...
    [
      ...
      body("mobile").custom((value) => {
        if (/^0/.test(value)) {
          throw new Error("Mobile number start with 0");
        }
        if (!/^[1-9]\d{5,11}$/.test(value)) {
          throw new Error(
            "Mobile number between 6-12 digits"
          );
        }
        return true;
      }),
    ...
    async (req, res) => {
      ...
      try {
        ...
        // Send OTP via Twilio
        await sendOTPToMobile(mobile, otp,isdCode);
